	...
	switch (event) {
		case STATE_NOTIFIER_ACTIVE:
			if (suspend_flag)
			    zzmoove_resume();
#ifdef ZZMOOVE_DEBUG
			pr_info("[zzmoove/state_notifier] Device active.\n");
#endif /* ZZMOOVE_DEBUG */
			break;
		case STATE_NOTIFIER_SUSPEND:
			if (!suspend_flag)
			    zzmoove_suspend();
#ifdef ZZMOOVE_DEBUG
			pr_info("[zzmoove/state_notifier] Device suspended.\n");
#endif /* ZZMOOVE_DEBUG */
			break;
		default:
			break;
...
static int __init cpufreq_gov_dbs_init(void)						// ZZ: idle exit time handling
{
	...
	if (state_register_client(&notif))
		pr_err("Cannot register state notifier callback for zzmoove governor.\n");
#endif
